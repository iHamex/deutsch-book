# Part 01 - Verbs and Tenses (A2)

> Goal: narrate the past with full Präteritum, master difficult Perfekt forms, understand Plusquamperfekt, use Futur I, and be polite with Konjunktiv II.

---

## Files in this Part (read in order)

| # | File | What you learn | Time |
|---|---|---|---|
| 01 | **präteritum.md** (planned) | Full Präteritum: haben/sein/werden, all modals, weak verbs, strong verbs (ging, kam, gab, nahm, sah, fuhr, las, blieb), als + Präteritum stories | 60 min |
| 02 | **perfekt-deep.md** (planned) | Difficult Perfekt: Mischverben (gebracht, gedacht, gekannt, gewusst), inseparable prefixes (ver-/be-/er-/ent-/miss-/zer-), seit + present vs Perfekt | 45 min |
| 03 | **plusquamperfekt.md** (planned) | Receptive Plusquamperfekt: hatte/war + Partizip, nachdem + Plusquamperfekt, time order in stories | 30 min |
| 04 | **futur.md** (planned) | Futur I werden + Infinitiv, Vermutung (Er wird krank sein), promise vs plan | 30 min |
| 05 | **konjunktiv-II.md** (planned) | Konjunktiv II: hätte/wäre/würde/könnte/müsste/sollte, Wünsche, höfliche Bitten, Ratschläge | 60 min |
| 06 | **partizipien.md** (planned) | Partizip I/II as adjectives (der lachende Mann, das gekaufte Buch), receptive | 30 min |

---

## How this Part connects

- Before: A1 [03-Verbs](../../A1/03-Verbs/00-INDEX.md) (you know Perfekt basics and war/hatte/konnte)
- After: [02-Sentences-Clauses](../02-Sentences-Clauses/00-INDEX.md) (use past forms inside Nebensätze)
- Practice: after 01-präteritum, narrate your weekend in 08-Practice-Lab.

## What you can do after this Part

- Tell a story in the past: Als ich klein war, spielte ich jeden Tag Fußball.
- Use difficult participles: Ich habe das vergessen / Er hat mir geholfen (review) / Sie hat es mitgebracht.
- Be polite: Ich hätte gern ein Ticket / Würden Sie mir bitte helfen?
- Guess the present: Er wird schon zu Hause sein.

## Tip

Präteritum is for NARRATION (stories, reports). Perfekt is for CONVERSATION (what you did). At A2 you need both - the exam tests both.
